...
public class RemoveVoteForPhoto {
    ...
    private static String removeVote;
    private Response.Listener<JSONObject> listener;
    private Map<String, String> params;

    public static String removeVoteOfPhoto(String userId, String photoListid, String categoryOfPhoto) throws Exception {
        method = "unVoteToPhoto";
        format = "json";
        idOfUser = userId;
        PhotoId = photoListid;
        ...
        JsonObjectRequest removeVoteRequest = new JsonObjectRequest(Request.Method.POST, URL, params,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        VolleyLog.v("Response:%n %s", response.toString());
                        try {
                            returnResponse(response.getString("saveUnVoteResponse"));
        ...
        removeVoteRequest.setRetryPolicy(new DefaultRetryPolicy(
                0,
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES,
                DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        AppController.getInstance().addToRequestQueue(removeVoteRequest);
        return removeVote;
    }

    public RemoveVoteForPhoto(Context context) {
        this.context = context;
    }

    public static void returnResponse(String response) {

        if (response.equals("UNVOTING_SUCCESS")) {
            Toast.makeText(context, "You successfully Remove your vote for these photo.", Toast.LENGTH_SHORT).show();
